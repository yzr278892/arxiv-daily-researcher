...
每日研究、补充报告、过去日报、趋势研究和关键词趋势均支持 HTML/Markdown。报告查看按实际批次前后浏览，同日多份报告不会被日期导航跳过；收藏、偏好和全文检索基于 SQLite 保存。
...
<table>
  ...
  <tr>
    ...
    <td align="center" width="33%">
      <img src="assets/webui_history_import_v4.png" alt="历史维护调度" width="100%" />
      <br />
      <sub>历史维护与运行方式</sub>
    </td>
  </tr>
...
| 扫描历史遗漏 | 扫描 SQLite 覆盖范围，按自然周形成补充积压 | 闲时或指定时间段 |
...
- 日报、补充报告和过去日报位于 `data/reports/daily_research/`；趋势报告和关键词趋势分别位于对应目录。
- 日报按文件时间戳排序。报告预览的“上一份报告 / 下一份报告”按实际批次移动，同一日期的多份报告均可访问。
- 预览中的 👍 / 👎 标记写入 SQLite，不修改归档 HTML。收藏页支持自动收藏后续合格论文，以及扫描并收藏现有合格论文。
