...
v4.5 使用 SQLite 保存候选、处理阶段、报告交付、通知发件箱、收藏偏好、历史维护积压与过去日报队列。补充报告独立归档，可将旧目录中的补充报告连同 SQLite 路径一次迁移。LLM 请求将稳定指令置于前缀以利用提供商缓存；Token 用量分别记录普通输入、缓存输入和输出，趋势图图例会为中英文标签预留正确宽度。任务可从已完成阶段恢复；运行配置与源码分离，便于长期部署、升级和备份恢复。Worker 打开研究数据库时会自动修复旧库中 SQLite 身份列与待处理论文 JSON 身份不一致的 DOI 记录，避免跨天队列中止每日研究。
...
根目录 `docker-compose.yml` 只用于实际部署，固定引用与 v4.5 Release 对应的官方镜像：

| 服务 | 镜像 | 网络与入口 |
| :--- | :--- | :--- |
| `arxiv-daily-researcher` | `ghcr.io/yzr278892/arxiv-daily-researcher:4.5` | 宿主机网络；cron、任务队列和 worker |
| `config-panel` | `ghcr.io/yzr278892/arxiv-daily-researcher-config-panel:4.5` | bridge 网络；`8501:8501` WebUI |
...
| 版本 | 日期 | 摘要 |
| :--- | :--- | :--- |
| **v4.5** | 2026-09-06 | 修复待处理 DOI 论文的 SQLite/JSON 身份迁移；已有不一致记录在 Worker 打开研究数据库时自动修复，不再中止每日研究。 |
| **v4.4** | 2026-09-02 | Worker 空队列时跳过 Python 任务选择器，例行健康检查改为轻量存活校验，降低待机 CPU 占用；用量趋势图中英文图例不再重叠。 |
